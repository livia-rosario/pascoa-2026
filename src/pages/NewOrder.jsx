...
const inp = {
  ...
  background: "#fdf4ef", outline: "none", fontFamily: "inherit",
};
const lbl = {
  display: "block", fontSize: 11, fontWeight: 800, color: "var(--muted)",
  marginBottom: 4, letterSpacing: .4, textTransform: "uppercase",
};
...
export default function NewOrder({ onNavigate }) {
  ...
  const addLine = () => setLines([...lines, { productId: "", qty: 1, filling: "" }]);
  ...
  if (!products.length) return <div style={{ textAlign: "center", padding: 40, color: "var(--muted)", fontWeight: 700 }}>Carregando produtos...</div>;
  ...
  return (
    <div>
      ...
      <div style={{ background: "var(--white)", borderRadius: "var(--radius)", padding: 16, border: "1.5px solid var(--border)", boxShadow: "var(--shadow)" }}>
          ...
          return (
            <div key={i} style={{ borderBottom: "1px solid var(--border)", paddingBottom: 10, marginBottom: 10 }}>
              <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}>
                <select value={line.productId || prod?._id} onChange={e => updateLine(i, "productId", e.target.value)}
                ...
                {lines.length > 1 && (
                  <button onClick={() => removeLine(i)} style={{ padding: "7px 10px", background: "#fff0ee", border: "2px solid #f5c0b8", color: "#c0392b", borderRadius: "var(--radius)", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>✕</button>
                )}
              </div>
              {prod?.hasFillings && (
        ...
        <button onClick={addLine} style={{ width: "100%", padding: "10px", background: "#fdf4ef", border: "2px solid var(--border)", color: "var(--text)", borderRadius: "var(--radius)", fontSize: 13, fontWeight: 800, cursor: "pointer", marginTop: 4 }}>
          + Adicionar produto
        </button>
